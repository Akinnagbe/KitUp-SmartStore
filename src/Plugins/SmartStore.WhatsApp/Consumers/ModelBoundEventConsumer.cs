...
namespace SmartStore.WhatsApp
{
    public class ModelBoundEventConsumer : IConsumer
    {
        ...
        {
            if (!eventMessage.BoundModel.CustomProperties.ContainsKey("WhatsApp"))
                return;

            var model = eventMessage.BoundModel.CustomProperties["WhatsApp"] as AdminEditTabModel;
            if (model == null)
                return;

            var settings = _settingService.LoadSetting<WhatsAppSettings>(_storeContext.CurrentStore.Id);
