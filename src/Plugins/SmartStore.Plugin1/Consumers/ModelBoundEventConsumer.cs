...
namespace SmartStore.Plugin1
{
    public class ModelBoundEventConsumer : IConsumer
    {
        ...
        {
            if (!eventMessage.BoundModel.CustomProperties.ContainsKey("Plugin1"))
                return;

            var model = eventMessage.BoundModel.CustomProperties["Plugin1"] as AdminEditTabModel;
            if (model == null)
                return;

            var settings = _settingService.LoadSetting<Plugin1Settings>(_storeContext.CurrentStore.Id);
